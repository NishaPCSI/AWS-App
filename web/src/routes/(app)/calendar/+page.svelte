<script lang="ts">
  ...
  import { _getMonthCalendar } from "./+page";
  import Calendar from "../../../components/Calendar.svelte";
  import type { CalendarDay } from "../../../types/CalendarDay";

  export const goto_id = (d: CalendarDay) => {
    document
      .querySelector(`#_${d.date.getTime()}`)
    ...
    for (let i = 0; i < daysInThisMonth; i++) {
      ...
      if (i == 0)
        days.push({
          ...
          enabled: true,
          date: d,
          tasks: data.task_data[d.getTime()] ?? [],
        });
      else
        days.push({
          name: "" + (i + 1),
  ...
  const mark_as_done = async (date: string, tid: string) => {
    ...
    if (resp.ok) {
      data.task_data[date].filter((x) => {
        return x.tid === tid;
      })[0].status = TaskStatus.Finished;
...
<div class="calendar-container">
  ...
  <Calendar {headers} {days} day_click_callback={goto_id} />
</div>

{#each Object.entries(data.task_data).sort() as [date, task] (date)}
  {@const _date = new Date(Number(date)).toLocaleString("en-US", {
    day: "numeric",
    weekday: "short",
  })}
  <div class="grid-list">
    ...
    <div class="card-grid-container">
      {#each task as t (t.tid)}
        <CardContainer
          ...
          btn_pri="Done"
          btn_pri_callback={t.status !== TaskStatus.Finished
            ? () => {
                mark_as_done(date, t.tid);
              }
            : undefined}
          btn_sec="Edit"
          btn_sec_callback={() => {
